...
def search_youtube(query: str) -> str:
    """
    Return the URL of the YouTube result with the most views
    that has 'lyrics' in the title. Falls back to most-viewed
    overall if no lyrics video is found.
    """
    # ytsearch10 tells yt-dlp to return 10 results instead of 1
    ydl_opts = {"quiet": True, "skip_download": True}
    with YoutubeDL(ydl_opts) as ydl:
        info = ydl.extract_info(f"ytsearch10:{query} lyrics", download=False)

        if not info["entries"]:
            raise ValueError(f"No YouTube results for {query!r}")

        # Filter entries to only those with 'lyrics' in the title (case-insensitive)
        lyrics_entries = [
            e for e in info["entries"]
            if e and "lyrics" in e.get("title", "").lower()
        ]

        # If we found lyrics videos, pick the one with the most views
        # If not, fall back to the most-viewed result from all 10
        pool = lyrics_entries if lyrics_entries else info["entries"]
        best = max(pool, key=lambda e: e.get("view_count", 0) or 0)
